...
void ase_data::readGenotypes2(string filename, string str_regions ,string olog) {
	...
	while(bcf_sr_next_line (sr)) {
		...
		if (line->n_allele > 2) {
			n_excludedG_mult ++;
			if (olog != "") fdo << "VCF_MULTI_ALLELIC " << sid << endl;
		} else {
			bcf_unpack(line, BCF_UN_STR);
			if (!filter_genotype.check(sid)) {
				n_excludedG_user ++;
				if (olog != "") fdo << "VCF_USER " << sid << endl;
			} else {
				...
				if(ase_basic_block(curr_chr,pos+1,pos+1).find_this_in_bool(blacklisted_regions)){
					n_excludedG_blkl++;
					if (olog != "") fdo << "VCF_BLACKLIST " << sid << endl;
				}else{
					string sid = string(line->d.id);													//sid
					string ref = string(line->d.allele[0]);												//ref
					string alt = string(line->d.allele[1]);												//alt
					niq = bcf_get_info_float(sr->readers[0].header, line, param_imputation_score_label.c_str(), &iq_arr, &niq_arr);		//imp score

					if (ref.size() > 1 || alt.size() > 1) {
						n_excludedG_snpv ++;
						if (olog != "") fdo << "VCF_INDEL " << sid << endl;
						...
						ngp = bcf_get_format_float(sr->readers[0].header, line,param_genotype_likelihood_label.c_str(), &gp_arr, &ngp_arr);
						if (ngt != n_samples_in_file * 2){
							n_excludedG_void ++;
							if (olog != "") fdo << "VCF_MISSING_GT " << sid << endl;
						} else if (gt_arr[2*index_sample+0] == bcf_gt_missing || gt_arr[2*index_sample+1] == bcf_gt_missing) {
							n_excludedG_miss ++;
							if (olog != "") fdo << "VCF_MISSING_GENOTYPE " << sid << endl;
						} else if (ngp == 3 * n_samples_in_file && gp_arr[3*index_sample+0] != bcf_float_missing && gp_arr[3*index_sample+1] != bcf_float_missing && gp_arr[3*index_sample+2] != bcf_float_missing && gp_arr[3*index_sample+0] < param_min_gp && gp_arr[3*index_sample+1] < param_min_gp && gp_arr[3*index_sample+2] < param_min_gp) {
							n_excludedG_impp ++;
							if (olog != "") fdo << "VCF_BAD_GENOTYPE " << sid << endl;
						} else if (bcf_gt_allele(gt_arr[2*index_sample+0]) == bcf_gt_allele(gt_arr[2*index_sample+1])) {
							n_excludedG_homo ++;
							if (olog != "") fdo << "VCF_HOMOZYGOUS " << sid << endl;
						} else {
